...
compound = compound || app.compound;
compound.websockets = {};
exports.init = function(){
    ...
    for(var key in config){
        ...
        compound.websockets[config[key]] = wss;
        initConnection(wss,config[key]);
    }
    compound.websockets['count'] = i;
}

exports.create = function(acceptName){    
    if(compound.websockets[acceptName]){
        ...
        var wss = new WebSocketServer({
            httpServer: app,
            autoAcceptConnections: false,
            id : compound.websockets['count'],
            conns : {},
            count : 0
        });
        compound.websockets[acceptName] = wss;
        initConnection(wss,acceptName);
        compound.websockets['count']++;
    }     
}
...
function removeConnection(conn,acceptName){
    ...
    conn.disconnected();
    var wss = compound.websockets[acceptName];
